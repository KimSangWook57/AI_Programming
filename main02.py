...
def run_train(x, y):
    # 순방향 => 정확도
    # 단층신경망이라면 aux_nn가 y에 갔겠지...
    output, aux_nn = forward_neuralnet(x) # x는 언제 씀? => '정확도 계산'
    loss, aux_pp = forward_postproc(output, y) # loss랑 diff는 언제 씀?
    # 이것은 지도학습이다. # 비지도라면 x'(프라임)을 넣었을 것.
    accuracy = eval_accuracy(output, y)
    
    # 역방향 => 오차
    G_loss = 1.0
    G_output = backprop_postproc(G_loss, aux_pp)
    backprop_neuralnet(G_output, aux_nn) # 기울기와 절편을 여기서 수정할 것이다.
...
def forward_postproc(output, y):
    diff = output - y # output - x # y^hat-y # 예측한 결과에서 y를 빼는 것.
...
def backprop_postproc(G_loss, diff): # aux_pp => x
    # 기울기 구하기
    shape = diff.shape
    # 편미분 실행
    g_loss_squre = np.ones(shape) / np.prod(shape)
    g_square_diff = 2 * diff
    g_diff_output = 1

    G_square = g_loss_squre * G_loss
